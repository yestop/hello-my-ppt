...
function usage() {
  console.log(
    "hello-my-ppt CLI\n\n" +
      "用法:\n" +
      "  hello-my-ppt serve [--port <port>] [--project <目录>]  启动本地网页编辑器\n" +
      "      --project: 挂载任意项目目录到浏览器（?deck=project/deck.pptd），端口占用自动顺延\n" +
      "  hello-my-ppt export <deck.pptd> [-o <out.pptx>]  命令行导出 PPTX\n" +
      "                           [--no-embed-fonts]   不嵌入字体（默认嵌入）\n" +
      "  hello-my-ppt export-project <deck.pptd> [-o <out.zip>]  导出项目包（pptd+pages+media，原样打包）\n" +
      "  hello-my-ppt render <deck.pptd> [-o <目录>] [--page <n|all>] [--scale <1|2|3>]\n" +
      "                           [--browser <路径>] [--timeout <毫秒>]\n" +
      "                        逐页渲染为 PNG（无头浏览器，与编辑器预览同管线）\n" +
      "  hello-my-ppt gallery scan                      扫描 examples/ 生成静态画廊索引\n" +
      "                        （examples/manifest.json，仅提交给 GitHub Pages 用；本地 serve 自动扫描）\n" +
      "  hello-my-ppt gallery list                      列出画廊条目\n" +
      "\n" +
      "  字体库（assets/fonts/，全部免费商用，默认子集化嵌入）：\n" +
      "  hello-my-ppt fonts list                         查看内置字体库（状态 ✓/✗）\n" +
      "  hello-my-ppt fonts download <名称|all>          按需/全量下载字体文件到字体库\n" +
      "  hello-my-ppt fonts check <deck.pptd>            体检 deck 字体声明（嵌入/仅声明/缺失）\n"
  );
}
